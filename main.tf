...
resource "oci_devops_deploy_artifact" "this" {
  ...
  deploy_artifact_source {
    ...
    repository_id               = try(each.value.source.repository_id, null) != null ? each.value.source.repository_id : (each.value.source.repository_key == null ? null : oci_devops_repository.this[each.value.source.repository_key].id)
